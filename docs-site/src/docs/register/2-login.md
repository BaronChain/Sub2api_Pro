...
登录入口：<https://freecode.codes/login>
...
如果点击“使用 Google 继续”后出现“错误：OIDC 获取 Token 失败，请检查设置！”，通常是浏览器缓存或 Cookie 状态异常导致。可以先清空浏览器缓存后重试：

- **Windows / Linux Chrome**：按 `Ctrl + Shift + Delete` 打开清除浏览数据页面。
- **macOS Chrome**：按 `Command + Shift + Delete` 打开清除浏览数据页面。

如果清理缓存后仍然无效，请手动删除 `freecode.codes` 相关 Cookie：

1. 在登录页按 `F12` 打开开发者工具。
2. 进入“应用”面板。
3. 在左侧依次选择“存储” → “Cookie” → `https://freecode.codes`。
